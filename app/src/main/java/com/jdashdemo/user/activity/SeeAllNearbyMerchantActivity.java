...
import com.jdashdemo.user.models.AllMerchant;
...
public class SeeAllNearbyMerchantActivity extends AppCompatActivity {
    ImageView backbtn;
    TextView address;
    EditText search;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_see_all_merchant);
...
//                            }
    ...
    private void getdata(final Location location) {
        ...
        AllMerchantsRequestJson param = new AllMerchantsRequestJson();
        ...
        userService.merchants(param).enqueue(new Callback<AllMerchantsResponseJson>() {
            @Override
            public void onResponse(@NonNull Call<AllMerchantsResponseJson> call, @NonNull Response<AllMerchantsResponseJson> response) {
                if (response.isSuccessful()) {
                    if (Objects.requireNonNull(response.body()).getMessage().equalsIgnoreCase("success")) {
                        shimmertutup();

                        if (response.body().getMerchants().isEmpty()) {
                            rvmerchantnear.setVisibility(View.GONE);
                            nodatanear.setVisibility(View.VISIBLE);
                        } else {
                            clicknear = response.body().getMerchants();
                            merchantNearItem = new AllMerchantNearItem(SeeAllNearbyMerchantActivity.this, clicknear, R.layout.item_merchant_list);
                            rvmerchantnear.setAdapter(merchantNearItem);

//                            catMerchantNearItem = new CatMerchantNearItem(SeeAllNearbyMerchantActivity.this, response.body().getKategori(), R.layout.item_cat_merchant, new CatMerchantNearItem.OnItemClickListener() {
//                                @Override
...
//                                    nodatanear.setVisibility(View.GONE);
//                                    shimmerchantnear.startShimmerAnimation();
//                                    FusedLocationProviderClient mFusedLocation = LocationServices.getFusedLocationProviderClient(SeeAllNearbyMerchantActivity.this);
//                                    mFusedLocation.getLastLocation().addOnSuccessListener(SeeAllNearbyMerchantActivity.this, new OnSuccessListener<Location>() {
//                                        @Override
...
//                            });
            ...
            @Override
            public void onFailure(@NonNull Call<AllMerchantsResponseJson> call, @NonNull Throwable t) {
    ...
    private void searchmerchant(final Location location, String like) {
        ...
        UserService userService = ServiceGenerator.createService(
                UserService.class, loginUser.getNoTelepon(), loginUser.getPassword());
        SearchAllMerchantRequestJson param = new SearchAllMerchantRequestJson();
        param.setLatitude(String.valueOf(location.getLatitude()));
        param.setLongitude(String.valueOf(location.getLongitude()));
        param.setNoTelepon(loginUser.getNoTelepon());
        param.setLike(like);
        userService.searchallmerchant(param).enqueue(new Callback<SearchAllMerchantResponseJson>() {
            @Override
            public void onResponse(@NonNull Call<SearchAllMerchantResponseJson> call, @NonNull Response<SearchAllMerchantResponseJson> response) {
                if (response.isSuccessful()) {
                    if (Objects.requireNonNull(response.body()).getMessage().equalsIgnoreCase("success")) {
                        clicknear = response.body().getMerchants();
                        shimmerchantnear.setVisibility(View.GONE);
            ...
            @Override
            public void onFailure(@NonNull Call<SearchAllMerchantResponseJson> call, @NonNull Throwable t) {
