...
import com.jdashdemo.user.models.Notif;
...
public class TopupSaldoActivity extends AppCompatActivity {
    ...
    private LinearLayout grabpay,gcash;
...
//        banktransfer = findViewById(R.id.banktransfer);
        ...
        grabpay = findViewById(R.id.grabpay);
...
//        });
    ...
    private void paymmentGrabpay() {
        progressshow();
        paymentAmount = nominal.getText().toString();
        final User user = BaseApp.getInstance(this).getLoginUser();
        PayMongoRequestJson request = new PayMongoRequestJson();
        request.setId(user.getId());
        request.setSource("grab_pay");
        request.setName(user.getFullnama());
        request.setPhone(user.getPhone());
        request.setEmail(user.getEmail());
        int Amount = Integer.valueOf(convertAngka(nominal.getText().toString().replace(sp.getSetting()[4], "")));
        request.setAmount(Amount);

        PayMongoRequestJson.Address inner = request.new Address();
        inner.setLine1("");
        inner.setLine2("");
        inner.setState("");
        inner.setPostalCode("");
        ...
        request.setAddress(inner);
        Log.e("CHAR", request.toString());
        System.out.println(request);

        UserService service = ServiceGenerator.createService(UserService.class, user.getNoTelepon(), user.getPassword());
        service.paymongo(request).enqueue(new Callback<PayMongoResponseJson>() {
            @Override
            public void onResponse(Call<PayMongoResponseJson> call, Response<PayMongoResponseJson> response) {
                progresshide();
                if (response.isSuccessful()) {
                    if (Objects.requireNonNull(response.body()).getMessage().equalsIgnoreCase("Success")) {
                        ...
                        Intent i = new Intent(TopupSaldoActivity.this, GrabPayActivity.class);
                        i.putExtra("LINK", checkoutUrl);
                        i.putExtra("ID", id);
            ...
            @Override
            public void onFailure(Call<PayMongoResponseJson> call, Throwable t) {
                progresshide();
                t.printStackTrace();
                notif("error");
            }
    ...
    private void paymongoGcash() {
        progressshow();
        paymentAmount = nominal.getText().toString();
        final User user = BaseApp.getInstance(this).getLoginUser();
        PayMongoRequestJson request = new PayMongoRequestJson();
        request.setId(user.getId());
        request.setSource("gcash");
        request.setName(user.getFullnama());
        request.setPhone(user.getPhone());
        request.setEmail(user.getEmail());
        int Amount = Integer.valueOf(convertAngka(nominal.getText().toString().replace(sp.getSetting()[4], "")));
...
//
                        ...
                        String phone = response.body().getPhone();
                        String name = response.body().getName();
//                        String idSource = response.body().ge

                        Intent i = new Intent(TopupSaldoActivity.this, GCashActivity.class);
                        i.putExtra("LINK", checkoutUrl);
                        i.putExtra("ID", id);
                        i.putExtra("AMOUNT", amount);
                        i.putExtra("TYPE", type);
